...
CREATE TABLE payment_metadata (
    ...
    description TEXT,
    ticket_token VARCHAR(255) UNIQUE, -- Ссылка на QR-токен билета
...
CREATE TABLE refunds (
    ...
    is_simulated BOOLEAN DEFAULT FALSE, -- Флаг для симуляции
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    processed_at TIMESTAMP
);

-- Индексы для ускорения поиска
CREATE INDEX idx_payment_metadata_user_id ON payment_metadata(user_id);
CREATE INDEX idx_payment_metadata_ticket_token ON payment_metadata(ticket_token);
